...
debug_roi <- function(roi_index, ROIs, summits) {
  ...
  lvl1 <- GenomicRanges::reduce(split(nearby, nearby$set), min.gapwidth = 1)
...
# mcols(All_Samples.fwp.filter.non_overlapping)
```

### Looking at summit score distributions

```{r summit scores}
all_H3K27ac_summits_gr %>% 
  as.data.frame() %>%
...
```{r scores after merging}
consensus_summits_gr %>% 
  as.data.frame() %>% 
  ggplot(., aes(x=group, y=score))+
  geom_boxplot()#+
  coord_cartesian(ylim=c(0,100))
  ...
  consensus_summits_gr %>% 
  as.data.frame() %>% 
    dplyr::filter(group=="VEH_144R") %>%
    group_by(Individual) %>% tally
  ggplot(., aes(x=Individual, y=score))+
  geom_boxplot()+
  coord_cartesian(ylim=c(0,100))

doublelist <- complete_summit_gr %>% 
  as.data.frame() %>% 
  group_by(Peakid) %>% 
  tally() %>% 
  dplyr::filter(n>1)
