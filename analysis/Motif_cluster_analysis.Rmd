...
all_H3K27ac_regions %>%  
  ...
  left_join(., H3K27ac_lookup) %>% 
   tidyr::replace_na(list(cluster = "NA")) %>% 
  mutate(cluster=factor(cluster, levels = c("Set_1","Set_2","Set_3","NA"))) %>% 
  ggplot(., aes(x=cluster,y = after_stat(prop),fill=cluster, group=1))+
...
all_H3K27ac_regions %>%  
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      ...
      "H3K27ac ROI counts per cluster"
    ),
    options = list(
      pageLength = 10,
      autoWidth = TRUE,
...
#### Histone proportions
```{r H3K27me3 histone prop}
H3K27me3_lookup <- imap_dfr(peakAnnoList_H3K27me3[1:2], ~
  tibble(Peakid = .x@anno$Peakid, cluster = .y))
all_H3K27me3_regions %>%  
  mutate(group="H3K27me3") %>% 
  ...
  geom_text(aes( label = scales::percent(..prop..),
                   x= ..prop.. ), stat= "count", vjust = -.5)+
  ggtitle("H3K27me3 proportions of ROIs")+
  theme_classic()
...
all_H3K27me3_regions %>%  
  ...
  left_join(., H3K27me3_lookup) %>% 
   tidyr::replace_na(list(cluster = "NA")) %>% 
  mutate(cluster=factor(cluster, levels = c("Set_1","Set_2","NA"))) %>% 
  ggplot(., aes(x=cluster,y = after_stat(prop),fill=cluster, group=1))+
  geom_bar(aes(stat="count"))+
  geom_text(aes( label = scales::percent(after_stat(prop))),
            stat= "count", vjust = -.5)+
...
all_H3K27me3_regions %>%  
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      style = "caption-side: top; text-align: left;",
      "H3K27me3 ROI counts per cluster"
    ),
    options = list(
      pageLength = 10,
      autoWidth = TRUE,
...
#### Histone proportions
```{r H3K36me3 histone prop}
H3K36me3_lookup <- imap_dfr(peakAnnoList_H3K36me3[1:2], ~
  tibble(Peakid = .x@anno$Peakid, cluster = .y))
all_H3K36me3_regions %>%  
  mutate(group="H3K36me3") %>% 
  ...
  geom_text(aes( label = scales::percent(..prop..),
                   x= ..prop.. ), stat= "count", vjust = -.5)+
  ggtitle("H3K36me3 proportions of ROIs")+
  theme_classic()
...
all_H3K36me3_regions %>%  
  ...
  left_join(., H3K36me3_lookup) %>% 
   tidyr::replace_na(list(cluster = "NA")) %>% 
  mutate(cluster=factor(cluster, levels = c("Set_1","Set_2","NA"))) %>% 
  ggplot(., aes(x=cluster,y = after_stat(prop), group=1))+
  geom_bar(aes(stat="count"))+
  geom_text(aes( label = scales::percent(after_stat(prop))),
            stat= "count", vjust = -.5)+
  scale_y_continuous(
...
all_H3K36me3_regions %>%  
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      style = "caption-side: top; text-align: left;",
      "H3K36me3 ROI counts per cluster"
    ),
    options = list(
      pageLength = 10,
      autoWidth = TRUE,
...
all_H3K9me3_regions %>%  
  ...
  geom_text(aes( label = scales::percent(..prop..),
                   x= ..prop.. ), stat= "count", vjust = -.5)+
  ggtitle("H3K9me3 proportions of ROIs")+
  theme_classic()
...
all_H3K9me3_regions %>%  
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      style = "caption-side: top; text-align: left;",
      "H3K9me3 ROI counts per cluster"
    ),
    options = list(
      pageLength = 10,
      autoWidth = TRUE,
...
```{r H3K9me3 line plot LFC set 3}
  ...
  ggplot(., aes(x=group, y=median_LFC, group=set, color=set))+
  geom_point(size=4)+
  geom_line(aes(alpha = 0.8, linewidth=3))+
  theme_bw()+
...
```{r exporting sets, eval=FALSE}
peakAnnoList_H3K36me3 <- readRDS("data/motif_lists/H3K36me3_annotated_peaks.RDS")

out_dir <- "data/Bed_exports/H3K36me3_sets"
...
for (group_name in set_list) {
  ...
  # Export to BED
 bed_file <- file.path(out_dir, paste0(group_name, "_H3K36me3_ROI.bed"))
