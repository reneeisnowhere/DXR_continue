...
K9me3_rmid_lfc <-
data_frame(RM_id=common_LTR_RMs) %>% 
  left_join(.,(LTR_overlap_df_H3K9me3 %>% 
    ...
    dplyr::select(RM_id, K9me3_Peakid))) %>% 
  left_join(K9me3_lfctable, by =c("K9me3_Peakid"="genes")) %>% 
  left_join(., H3K9me3_lookup,by =c("K9me3_Peakid"="Peakid")) %>% 
  group_by(RM_id) %>% 
  summarize(K9me3_24T=mean(H3K9me3_24T, na.rm=TRUE),
             K9me3_24R=mean(H3K9me3_24R,na.rm=TRUE),
             K9me3_144R=mean(H3K9me3_144R, na.rm=TRUE),
...
LTR_RM_lfc_concordance %>% 
  ggplot(., aes(x=K27ac_24T, y=K9me3_24T, color=cluster.x))+
...
LTR_RM_lfc_concordance %>% 
  ggplot(., aes(x=K27ac_24R, y=K9me3_24R, color=cluster.x))+
  geom_point(, alpha=0.2, size=2)+
  geom_density_2d(color= "black")+
   # sm_statCorr(corr_method="spearman")+
   geom_hline(yintercept = 0, linetype = "dashed", color="red") +
...
LTR_RM_lfc_concordance %>% 
  ggplot(., aes(x=K27ac_144R, y=K9me3_144R, color=cluster.x))+
  geom_point(, alpha=0.2, size=2)+
  geom_density_2d(color= "black")+
     # sm_statCorr(corr_method="spearman")+
...
LTR_RM_lfc_concordance %>% 
  ggplot(., aes(x=K27ac_24T, y=K27ac_24R, color=cluster.x))+
  geom_point(, alpha=0.2, size=2)+
  geom_density_2d(color= "black")+
   # sm_statCorr(corr_method="spearman")+
   geom_hline(yintercept = 0, linetype = "dashed", color="red") +
...
LTR_RM_lfc_concordance %>% 
  ggplot(., aes(x=K27ac_24T, y=K27ac_144R, color=cluster.x))+
  geom_point(, alpha=0.2, size=2)+
  geom_density_2d(color= "black")+
   # sm_statCorr(corr_method="spearman")+
   geom_hline(yintercept = 0, linetype = "dashed", color="red") +
...
LTR_RM_lfc_concordance %>% 
  ggplot(., aes(x=K27ac_24R, y=K27ac_144R, color=cluster.x))+
  geom_point(, alpha=0.2, size=2)+
  geom_density_2d(color= "black")+
   sm_statCorr(corr_method="spearman")+
   geom_hline(yintercept = 0, linetype = "dashed", color="red") +
...
K27_K9_family_plot <- function(lfc_df,
                            split_list, 
                            marks=c("K27ac","K9me3"),
                            time_levels = c("_24T", "_24R", "_144R"),
                            repClass,
                            repFam){

  if (!repFam %in% names(split_list)) {
  stop("repFam not found in split_list: ", repFam)}
  ...
  lfc_df %>% 
    dplyr::filter(RM_id %in% rm_ids) %>% 
    dplyr::rename(
    cluster_K27ac = tidyselect::any_of("cluster.x"),
    cluster_K9me3 = tidyselect::any_of("cluster.y")) %>% 
      tidyr::pivot_longer(
      cols = tidyselect::matches(paste0("^(", paste(marks, collapse = "|"), ")")),
      names_to = "group",
      values_to = "LFC"
    ) %>%
    dplyr::mutate(
      mark = stringr::str_extract(group, paste(marks, collapse = "|")),
      time = stringr::str_remove(group, paste(marks, collapse = "|")),
      time = factor(time, levels = time_levels),

      cluster = dplyr::case_when(
        mark == "K27ac" ~ cluster_K27ac,
        mark == "K9me3" ~ cluster_K9me3
      )
    ) %>%
    dplyr::filter(!is.na(cluster)) %>%
    dplyr::group_by(cluster, mark, time) %>%
    dplyr::summarise(
      median_abs_LFC = median(abs(LFC), na.rm = TRUE),
      n_RM = dplyr::n_distinct(RM_id),
      .groups = "drop") %>% 
  ggplot(., aes(x = time,
      y = median_abs_LFC,
      color = cluster,
      linetype = mark,
      shape = mark,
      group = interaction(cluster, mark))) +
    geom_line(linewidth = 1.1, alpha = 0.85) +
    geom_point(size = 3) +
    scale_linetype_manual(
  values = c("K9me3"  = "solid","K27ac" = "dotted"))+
    scale_color_manual(values = cluster_colors, drop = FALSE)+
    theme_bw() +
    labs(
      y = "Median |LFC|",
      x = "Time",
      title = paste0("Both histones, absolute LFC overlapping ",repClass,":",repFam))
}
...
K27_K9_family_plot(LTR_RM_lfc_concordance, LTR_split_df,marks=c("K27ac","K9me3"),time_levels = c("_24T", "_24R", "_144R"), "LTR",repFam="ERVL")

K27_K9_family_plot(LTR_RM_lfc_concordance, LTR_split_df,marks=c("K27ac","K9me3"),time_levels = c("_24T", "_24R", "_144R"), "LTR",repFam="ERVK")
K27_K9_family_plot(LTR_RM_lfc_concordance, LTR_split_df,marks=c("K27ac","K9me3"),time_levels = c("_24T", "_24R", "_144R"), "LTR",repFam="ERV1")
K27_K9_family_plot(LTR_RM_lfc_concordance, LTR_split_df,marks=c("K27ac","K9me3"),time_levels = c("_24T", "_24R", "_144R"), "LTR",repFam="ERVL-MaLR")
K27_K9_family_plot(LTR_RM_lfc_concordance, LTR_split_df,marks=c("K27ac","K9me3"),time_levels = c("_24T", "_24R", "_144R"), "LTR",repFam="Gypsy")
K27_K9_family_plot(LTR_RM_lfc_concordance, LTR_split_df,marks=c("K27ac","K9me3"),time_levels = c("_24T", "_24R", "_144R"), "LTR",repFam="LTR")


```
```{r, eval=FALSE}
rm_id_test <- LTR_split_df$ERV1$RM_id
LTR_RM_lfc_concordance %>% 
  dplyr::filter(RM_id %in% rm_id_test) %>% 
                
    dplyr::rename("cluster_K27ac"=cluster.x,
                  "cluster_K9me3"=cluster.y) %>% 
pivot_longer(
    cols = tidyselect::matches("^(K27ac|K9me3)"),
    names_to = "group",
    values_to = "LFC"
  ) %>% 
  mutate(
    mark = stringr::str_extract(group, "K27ac|K9me3"),
    time = stringr::str_remove(group, "K27ac|K9me3"),
    time = factor(time, levels = c("_24T","_24R","_144R")),

    ## mark-aware cluster assignment
    cluster = dplyr::case_when(
      mark == "K27ac" ~ cluster_K27ac,
      mark == "K9me3" ~ cluster_K9me3
    )
  ) %>% 
  filter(!is.na(cluster)) %>%   # only drop when THAT mark has no cluster
  ...
  labs(
    y = "Median |LFC|",
    x = "Time",
    title = "Shared LTR:ERV1: mark-specific cluster behavior"
  )
