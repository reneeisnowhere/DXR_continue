---
title: "H3K27me3 Extended summits and TE overlap"
...
# sat_lowc_simrpt_list <- repeatmasker_clean %>% dplyr::filter(repClass %in% c("Simple_repeat","	Low_complexity","Satellite")) %>% 
#   group_by(repName) %>% pull(repName) %>% unique()
# 
# saveRDS(sat_lowc_simrpt_list, "data/RDS_files/sat_lowc_simrpt_repNames.RDS")
```
...
```{r loading H3K27me3 data}
H3K27me3_summit_gr <- readRDS("data/RDS_files/H3K27me3_complete_summit_gr.RDS")
H3K27me3_summit_gr <- H3K27me3_summit_gr %>% 
  dplyr::filter(cluster != "all_H3K27me3_regions")
...
##assigning Peakid as name of summit region
 ...
 H3K27me3_summit_gr_ext <- resize(H3K27me3_summit_gr, width(H3K27me3_summit_gr)+400, fix = "center")
...
dataf <- H3K27me3_summit_gr_ext
  ...
  All_H3K27me3 <-peakAnnoList_H3K27me3$all_H3K27me3_regions

  annotated_H3K27me3_ext200 <-
  All_H3K27me3 %>% 
      as.data.frame() %>% 
    dplyr::select(Peakid,annotation,geneId:distanceToTSS) %>% 
     left_join(., (H3K27me3_ext200_ols %>% 
                  as.data.frame() %>% 
                  group_by(Peakid) %>% 
                  summarize(., 
...
# saveRDS(annotated_H3K27me3_ext200,"data/TE_annotation/annotated_H3K27me3_ext200_TE_overlaps.RDS")
```
...
```{r SINE Overlapping}
sine_hits <- findOverlaps(H3K27me3_summit_gr_ext, SINE_gr, ignore.strand = TRUE)
...
total_SINE_summits <- tibble(
  cluster = mcols(H3K27me3_summit_gr_ext)$cluster
...
```{r LINE Overlapping}
LINE_hits <- findOverlaps(H3K27me3_summit_gr_ext, LINE_gr, ignore.strand = TRUE)
LINE_overlap_df <- tibble(
  summit_id = queryHits(LINE_hits),
  cluster   = mcols(H3K27me3_summit_gr_ext)$cluster[queryHits(LINE_hits)],
...
```{r LTR Overlapping}
LTR_hits <- findOverlaps(H3K27me3_summit_gr_ext, LTR_gr, ignore.strand = TRUE)
...
```{r SVA Overlapping}
SVA_hits <- findOverlaps(H3K27me3_summit_gr_ext, SVA_gr, ignore.strand = TRUE)
...
```{r DNA Overlapping}
DNA_hits <- findOverlaps(H3K27me3_summit_gr_ext, DNA_gr, ignore.strand = TRUE)
...
not_DNA_PiggyBac_counts <- DNA_PiggyBac_counts %>%
  left_join(total_DNA_summits, by = "cluster") %>%
  mutate(count = total - count,
         status = "not_TE") %>%
  dplyr::select(cluster, repName, status, count)

DNA_PiggyBac_df_long <- bind_rows(DNA_PiggyBac_counts %>%
                            dplyr::select(cluster, repName, status, count),
                          not_DNA_PiggyBac_counts) %>% 
  filter(!is.na(cluster))

PiggyBac_DNA_results <- comparisons %>%
  mutate(results = map2(cluster2, cluster1, function(c2, c1) {

       DNA_PiggyBac_df_long %>%
        distinct(repName) %>%
        pull() %>%
        map_dfr(function(rep) {
          test_pair_TE_repName(
...
ggplot(PiggyBac_DNA_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  geom_point(aes(color = odds_ratio > 1), size = 3) +
  # geom_text_repel(data = subset(SVA_results, FDR < 0.05)) +
  geom_text_repel(data = subset(PiggyBac_DNA_results, FDR < 0.05),  # only significant points
    size = 3,
    max.overlaps = 100
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
  labs(
    x = "(Odds Ratio)",
    y = "-log10(FDR)",
    title = "DNA Name (PiggyBac) enrichment at ROI extended-summits\nH3K27me3"
  ) +
  theme_classic()+
  facet_wrap(~comparison)
```
...
anno_H3K27me3_ext200 <- readRDS("data/TE_annotation/annotated_H3K27me3_ext200_TE_overlaps.RDS")
anno_H3K27me3_ext200 %>% 
  dplyr::filter(cluster != "all_H3K27me3_regions") %>% 
...
H3K27me3_te_summary <- anno_H3K27me3_ext200 %>% 
  dplyr::filter(cluster != "all_H3K27me3_regions") %>% 
...
```{r classROI}
H3K27me3_te_family_long <- H3K27me3_te_summary %>% 
...
fam_pie_df <- H3K27me3_te_family_long %>% 
...
ggplot(fam_pie_df, aes(x = "", y = percent, fill = repClass)) +
  ...
  labs(
    title = "TE Class composition among summit-ext TE-overlapping H3K27me3 ROIs",
    fill = "TE Class")
...
H3K27me3_te_family_clust <- H3K27me3_te_family_long %>% 
...
# ggplot(H3K27me3_te_family_clust, aes(x = "", y=percent, fill=repClass))+
#   geom_col(width =1, color="white")+
#   coord_polar(theta = "y")+
#   geom_text(
...
ggplot(H3K27me3_te_family_clust, aes(x = cluster, y=percent, fill=repClass))+
  ...
  theme_bw() +
  labs(
    title = "TE Class composition among H3K27me3 summit-exit TEs per set",
    fill = "TE family"
  )
  ggplot(H3K27me3_te_family_clust, aes(x = cluster, y=percent, fill=repClass))+
   geom_col(width = 0.8, color = "white") +
...
# ggplot(H3K27me3_long_repFamily, aes(x = "", y=percent, fill=repFamily))+
#   geom_col(width =1, color="white")+
#   coord_polar(theta = "y")+
#     geom_text_repel(
...
H3K27me3_long_repFamily_ext <-H3K27me3_long_repFamily %>% 
  rbind((LTR_genome %>% mutate(cluster="hg38")))

H3K27me3_long_repFamily_ext
```

```{r LTR barplot}

LTR_bar_df <-H3K27me3_te_summary %>%
  filter(!is.na(repFamily), repFamily != "")  %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
  distinct(cluster,Peakid, repFamily) %>% 
  dplyr::filter(repFamily %in% c("ERV1", "ERVK","ERVL", "ERVL-MaLR","Gypsy","LTR")) %>%
...
later calculations required removing certain groups (satellites and simple repeats)

```{r}
annotated_H3K27me3_ext200 <- readRDS("data/TE_annotation/annotated_H3K27me3_ext200_TE_overlaps.RDS")

sat_lowc_simrpt_repNames <- readRDS("data/RDS_files/sat_lowc_simrpt_repNames.RDS")
repName_counts <- annotated_H3K27me3_ext200 %>%
...
ggplot(repName_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  ...
  labs(
    x = "(Odds Ratio)",
    y = "-log10(FDR)",
    title = "all repName enrichment at H3K27me3 extended ROI summits"
  ) +
  theme_classic() +
  facet_wrap(~comparison)
