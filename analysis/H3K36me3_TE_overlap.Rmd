...
```{r SINE Overlapping}
sine_hits <- findOverlaps(H3K36me3_summit_gr, SINE_gr, ignore.strand = TRUE)
...
total_SINE_summits <- tibble(
  cluster = mcols(H3K36me3_summit_gr)$cluster
...
```{r LINE Overlapping}
LINE_hits <- findOverlaps(H3K36me3_summit_gr, LINE_gr, ignore.strand = TRUE)
LINE_overlap_df <- tibble(
  summit_id = queryHits(LINE_hits),
  cluster   = mcols(H3K36me3_summit_gr)$cluster[queryHits(LINE_hits)],
...
```{r LTR Overlapping}
LTR_hits <- findOverlaps(H3K36me3_summit_gr, LTR_gr, ignore.strand = TRUE)
...
```{r SVA Overlapping}
SVA_hits <- findOverlaps(H3K36me3_summit_gr, SVA_gr, ignore.strand = TRUE)
...
```{r DNA Overlapping}
DNA_hits <- findOverlaps(H3K36me3_summit_gr, DNA_gr, ignore.strand = TRUE)
