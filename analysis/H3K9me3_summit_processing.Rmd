...
# saveRDS(temp_highest_per_group,"data/RDS_files/H3K9me3_temp_highest_per_group.RDS")

temp_highest_per_group <- readRDS("data/RDS_files/H3K9me3_temp_highest_per_group.RDS")

# flat_highest_gr <- do.call(c, temp_highest_per_group)
# # Concatenate into one GRanges (Step 1.2)
# old_levels <- seqlevels(flat_highest_gr)
# genome_info <- Seqinfo(
#   seqnames = paste0("chr", c(1:22, "X", "Y")),
#   seqlengths = NA,
#   genome = "hg38"
# )
# genome_info <- genome_info[old_levels]
# temp_highest_per_group <- lapply(temp_highest_per_group, function(gr) {
#   seqinfo(gr) <- genome_info
#   gr
# })
# 
# flat_highest_gr <- do.call(base::c, temp_highest_per_group)
...
# saveRDS(consensus_summits_gr,"data/RDS_files/H3K9me3_consensus_summits_gr.RDS")
consensus_summits_gr<- readRDS("data/RDS_files/H3K9me3_consensus_summits_gr.RDS")

# final_data <- assign_best_summit_to_ROI_parallel(consensus_summits_gr, ROIs,max_dist = 100,workers = workers)

# saveRDS(final_data,"data/RDS_files/H3K9me3_final_data.RDS")

final_data <- readRDS("data/RDS_files/H3K9me3_final_data.RDS")
...
```{r, eval=FALSE}
final_df$inside_roi <- with(final_df, summit_pos >=roi_start & summit_pos <= roi_end)

table(final_df$inside_roi, useNA= "ifany")

final_df$fallback_assigned <- with(final_df,
                             !is.na(summit_pos) &
                             !(summit_pos >= roi_start &
                               summit_pos <= roi_end))
table(final_df$fallback_assigned, useNA = "ifany")
```
...
# saveRDS(complete_summit_df,"data/RDS_files/H3K9me3_complete_summit_df.RDS")
...
# saveRDS(complete_summit_gr,"data/RDS_files/H3K9me3_complete_summit_gr.RDS")
...
complete_summit_gr <- readRDS("data/RDS_files/H3K9me3_complete_summit_gr.RDS")
library(BSgenome.Hsapiens.UCSC.hg38)
...
#   rtracklayer::export(for_saving,"data/Bed_exports/H3K9me3_summit_gr_ext200.bed")
```
