...
facet_name <- facet_levels[page]
  ##plotting the data
   ggplot(plot_df, aes(y = tidytext::reorder_within(motif_name_wrapped, log10Evalue, Facets))) +
    geom_col(aes(x = log10Evalue)) +
...
#                            title = NULL,
#                            signif.num = 0.05,
#                            top_n = NULL,       # optional: only show top N motifs per page
#                            page = 1) {
# 
...
# 
#   # 6️⃣ Plot
#   p <- ggplot(page_df, aes(y = tidytext::reorder_within(motif_name, log10Evalue, Facets))) +
#     geom_col(aes(x = log10Evalue)) +
