...
```{r apply the function, eval=TRUE}
...
temp_highest_per_group <- readRDS("data/RDS_files/H3K27ac_temp_highest_per_group.RDS")
...
```{r assigning missing}
roi_df <- final_df %>%
  filter(is.na(summit_pos)) %>%
  mutate(
    roi_seqname = roi_seqname,
    roi_start = roi_start,
    roi_end = roi_end,
    Peakid = Peakid
  ) %>%
  filter(!is.na(roi_seqname), !is.na(roi_start), !is.na(roi_end))

roi_gr <- GRanges(
  seqnames = roi_df$roi_seqname,
  ranges = IRanges(roi_df$roi_start, roi_df$roi_end)
)

peak_gr <- flat_highest_gr

ov <- findOverlaps(roi_gr, peak_gr)

overlap_df <- tibble(
  roi_idx = queryHits(ov),
  summit_idx = subjectHits(ov),

  # ROI identity (NEVER re-index GRanges)
  Peakid = roi_df$Peakid[queryHits(ov)],
  roi_seqname = roi_df$roi_seqname[queryHits(ov)],
  roi_start = roi_df$roi_start[queryHits(ov)],
  roi_end = roi_df$roi_end[queryHits(ov)],

  # summit info
  summit_pos = start(peak_gr)[subjectHits(ov)],
  summit_score = mcols(peak_gr)$score[subjectHits(ov)]
)

best_summits <- overlap_df %>%
  group_by(Peakid) %>%
  slice_max(summit_score, n = 1, with_ties = FALSE) %>%
  ungroup()

assigned_df_calc <- best_summits %>%
  mutate(
    dist_center = summit_pos - (roi_start + (roi_end - roi_start)/2),
    rel_pos = (summit_pos - roi_start) / (roi_end - roi_start)
  )
...
## sanity check
# only_complete %>% 
#   dplyr::filter(is.na(roi_seqname))
complete_summit_gr <- GRanges(
  seqnames = complete_summit_df$roi_seqname,
  ranges = IRanges(
    start = complete_summit_df$summit_pos,
    end   = complete_summit_df$summit_pos))

mcols(complete_summit_gr) <- complete_summit_df %>%
  dplyr::select(
    Peakid,
    roi_start,
    roi_end,
...
resize_and_trim <- function(gr, flank = 200, genome = BSgenome.Hsapiens.UCSC.hg38) {
  ...
  seqlengths(gr) <- seqlengths(genome)[intersect(seqlevels(gr), seqlevels(genome))]
...
# }
  ...
  # for_saving <- resize_and_trim(complete_summit_gr, flank=200)
  # rtracklayer::export(for_saving,"data/Bed_exports/H3K27ac_summit_gr_ext200.bed")
```
