...
combo_trim_df %>% 
  ...
  ylab("sequene count")+
  ggtitle("Paired End reads by sample and histone mark trimmed adapters")+
...
geom_boxplot(aes(fill=Histone_Mark)) + 
  ...
  ylab("count")+
  theme(axis.text.x=element_text(vjust = .2,angle=90))+
  ggtitle("Paired end read count by trt and time, trimmed adapter")
```
...
#### Data Initialization
```{r read noM data, message=FALSE, warning=FALSE}
