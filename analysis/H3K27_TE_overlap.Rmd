...
set_list <- unique(SINE_gr$repFamily)
 split_SINE_gr <- split(SINE_gr, mcols(SINE_gr)$repFamily)


for (group_name in set_list) {
  
 gr <- split_SINE_gr[[group_name]]
  ...
  # Export to BED
 bed_file <- file.path(out_dir, paste0(group_name, "_rptmasker_SINE.bed"))
...
out_dir <- "data/Bed_exports/repeatmasker_sets/MIR"


split_SINE_gr <- split(SINE_gr, mcols(SINE_gr)$repFamily)
set_list <- unique(split_SINE_gr$MIR$repName)
split_SINE_MIR_gr <- split_SINE_gr$MIR
split_MIR_gr <- split(split_SINE_MIR_gr, mcols(split_SINE_MIR_gr)$repName)


for (group_name in set_list) {
  
 gr <- split_MIR_gr[[group_name]]
  ...
  # Export to BED
 bed_file <- file.path(out_dir, paste0(group_name, "_MIR_SINE.bed"))
