...
### All H3K27ac ROIs overlapping a TE at summits

overlapping summits and all TEs dataframe making:

```{r}

datafa <- H3K27ac_summit_gr
# H3K27ac_summit_ols <-
  
   # 1️⃣ find overlaps
  hitsfa <- findOverlaps(datafa, rmskr_std_gr)
  
  if (length(hitsfa) == 0) {
  H3K27ac_summit_ols <- tibble()
} else {

  H3K27ac_summit_ols <- tibble(
    Peakid     = datafa$Peakid[queryHits(hitsfa)],
    cluster    = datafa$cluster[queryHits(hitsfa)],
    seqnames   = as.character(seqnames(datafa))[queryHits(hitsfa)],
    summit_pos = start(datafa)[queryHits(hitsfa)],

    repName   = rmskr_std_gr$repName[subjectHits(hitsfa)],
    repFamily = rmskr_std_gr$repFamily[subjectHits(hitsfa)],
    repClass  = rmskr_std_gr$repClass[subjectHits(hitsfa)],
    milliDiv  = rmskr_std_gr$milliDiv[subjectHits(hitsfa)],
    milliDel  = rmskr_std_gr$milliDel[subjectHits(hitsfa)],
    milliIns  = rmskr_std_gr$milliIns[subjectHits(hitsfa)],
    ID  = rmskr_std_gr$id[subjectHits(hitsfa)]
  )
}
# rmskr_nonoverlapping_H3K27ac_gr <- rmskr_std_gr[countOverlaps(rmskr_std_gr, H3K27ac_sets_gr[[4]]) == 0]
  
H3K27ac_summit_Peakids <- H3K27ac_summit_ols %>% 
    distinct(Peakid)
  # saveRDS(H3K27ac_summit_ols,"data/RDS_files/H3K27ac_summit_ROI_TE_overlaps.RDS")
...
```{r}
all_summits_df <- 
datafa %>% as.data.frame() %>% 
  mutate(summit_id=paste0(seqnames,":",start)) %>% 
  dplyr::select(summit_id, Peakid:cluster)


All_H3K27ac <- H3K27ac_sets_gr$all_H3K27ac %>% as.data.frame()
annotated_H3K27ac_summits <- All_H3K27ac %>%
    left_join(., (H3K27ac_summit_ols %>% 
                  as.data.frame() %>% 
                  group_by(Peakid) %>% 
                  summarize(., 
  ...
  mutate(cluster = if_else(is.na(cluster), "not_assigned", cluster)) %>% left_join(all_summits_df,by =c("Peakid"="Peakid","cluster"="cluster")) 

# saveRDS(annotated_H3K27ac_summits,"data/TE_annotation/H3K27ac_annotated_summit_TE_overlaps.RDS")
```


```{r enrichment across TEs summit version}
te_cols <- c("TE_status","SINE_status","LINE_status","DNA_status","LTR_status","SVA_status")
H3K27ac_counts_summits <- map(te_cols, ~ {
...
H3K27ac_counts_summits_long <- map2_dfr(
  H3K27ac_counts_summits,      # the named list of wide tables
  names(H3K27ac_counts_summits), # names of TE columns
  ~ .x %>%
    pivot_longer(
...
H3K27ac_counts_summits_long <-
  H3K27ac_counts_summits_long %>% 
group_by(TE_type) %>%
  tidyr::complete(cluster, status, fill = list(count = 0)) %>%
  ungroup()
H3K27ac_count_summits_long_fixed <- 
H3K27ac_counts_summits_long %>% 
...
```{r enrichment across TEs summit version plot}
TE_types <- te_cols


H3K27ac_summit_results_1bp <- map_dfr(TE_types, function(te){
  map_dfr(cluster_pairs_3, function(pair){
    test_pair_TE_generic(H3K27ac_count_summits_long_fixed, te, cluster1 = pair[2], cluster2 = pair[1])
  })
})
H3K27ac_summit_results_1bp%>% 
...
```{r making bed file subsets of LTRs, eval=FALSE}
out_dir <- "data/Bed_exports/repeatmasker_sets"
set_list <- unique(LTR_gr$repFamily)
split_LTR_gr <- split_LTR_gr <- split(LTR_gr, mcols(LTR_gr)$repFamily)


for (group_name in set_list) {
  
 gr <- split_LTR_gr[[group_name]]
 
  
  # Set BED name column
  mcols(gr)$name <- mcols(gr)$RM_id
  ...
  # Export to BED
 bed_file <- file.path(out_dir, paste0(group_name, "_rptmasker_LTR.bed"))
...
```{r making bed file subsets of LTRERV1, eval=FALSE}
out_dir <- "data/Bed_exports/repeatmasker_sets/ERV1"


split_LTR_gr <- split(LTR_gr, mcols(LTR_gr)$repFamily)
set_list <- unique(split_LTR_gr$ERV1$repName)
split_LTR_ERV1_gr <- split_LTR_gr$ERV1
split_ERV1_gr <- split(split_LTR_ERV1_gr, mcols(split_LTR_ERV1_gr)$repName)


for (group_name in set_list) {
  
 gr <- split_ERV1_gr[[group_name]]
  ...
  # Export to BED
 bed_file <- file.path(out_dir, paste0(group_name, "_ERV1_LTR.bed"))
...
```
### TE proportions by ROI and ROI-summits
...
H3K27ac_te_summary_summits <- anno_H3K27ac_summits %>% 
  left_join(H3K27ac_set_case_lookup) %>% 
   mutate(repClass = na_if(repClass, ""),   # treat empty as NA
    n_TE_class = case_when(
      is.na(repClass) ~ 0L,
...
pie_df_summits <- H3K27ac_te_summary_summits %>% 
  count(category) %>% 
  mutate(
    percent = n / sum(n) * 100
  )
...
```{r class summit}
H3K27ac_te_family_long_summits <- H3K27ac_te_summary_summits %>% 
  filter(!is.na(repClass), repClass != "") %>%   # TE-positive only
  separate_rows(repClass, sep = ";") %>% 
  distinct(Peakid, repClass,cluster)


fam_pie_df_summits <- H3K27ac_te_family_long_summits %>% 
  count(repClass, name = "n") %>% 
  mutate(percent = n / sum(n)*100) %>% 
  mutate(
    repClass = if_else(percent < 2, "Other", repClass)
...
H3K27ac_te_summary_summits
```

### TE LTR family distribution  


```{r Family LTR breakdown}
...
```{r family LTR summit breakdown}
H3K27ac_long_repFamily_summits <- H3K27ac_te_summary_summits %>% 
  filter(!is.na(repFamily), repFamily != "") %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
  distinct(Peakid, repFamily)


H3K27ac_LTR_pie_df_summits <- H3K27ac_long_repFamily_summits %>% 
  dplyr::filter(repFamily %in% c("ERV1", "ERVK","ERVL", "ERVL-MaLR","Gypsy","LTR")) %>%
  count(repFamily, name = "n") %>% 
  mutate(percent= n/ sum(n) * 100)

ggplot(H3K27ac_LTR_pie_df_summits, aes(x = "", y = percent, fill = repFamily)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  theme_void() +
  geom_text_repel(
...
```{r overlapping TE bar summit data}

H3K27ac_te_family_clust_summits <- H3K27ac_te_family_long_summits %>% 
  dplyr::filter(cluster != "not_assigned") %>%
   distinct(Peakid, cluster, repClass) %>%
    count(cluster, repClass, name = "n") %>% 
  group_by(cluster) %>% 
...
ggplot(H3K27ac_te_family_clust_summits, aes(x = "", y=percent, fill=repClass))+
  geom_col(width =1, color="white")+
  coord_polar(theta = "y")+
  geom_text_repel(
    aes(x= 1.5,label = sprintf("%s\n%.1f%%", repClass, percent)),
...
ggplot(H3K27ac_te_family_clust_summits, aes(x = cluster, y=percent, fill=repClass))+
   geom_col(width = 0.8, color = "white") +
  geom_text(
    aes(label = sprintf("%s\n%.1f%%", repClass, percent)),
    position = position_stack(vjust = 0.5),
...
H3K27ac_long_repFamily_summits <-
H3K27ac_te_summary_summits %>% 
   dplyr::filter(cluster != "not_assigned") %>%
  filter(!is.na(repFamily), repFamily != "") %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
...
ggplot(H3K27ac_long_repFamily_summits, aes(x = "", y=percent, fill=repFamily))+
  geom_col(width =1, color="white")+
  coord_polar(theta = "y")+
    geom_text_repel(
    aes(label = sprintf("%s\n%.1f%%", repFamily, percent)),
  ...
  theme_void()+
  labs(
    title = "TE family relative abundance among ROI summits by set",
    fill = "TE family"
  )


ggplot(H3K27ac_long_repFamily_summits, aes(x = cluster, y=percent, fill=repFamily))+
   geom_col(width = 0.8, color = "white") +
  geom_text_repel(
    aes(label = sprintf("%s\n%.1f%%", repFamily, percent)),
    position = position_stack(vjust = 0.5),
  ...
  theme_bw() +
  labs(
    title = "TE Family composition among TE-overlapping ROI summits per set",
    fill = "TE family"
  )
...
```{r LTR barplot summits}

LTR_bar_df_summits <-H3K27ac_te_summary_summits %>%
  filter(!is.na(repFamily), repFamily != "")  %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
  distinct(cluster,Peakid, repFamily) %>% 
  dplyr::filter(repFamily %in% c("ERV1", "ERVK","ERVL", "ERVL-MaLR","Gypsy","LTR")) %>%
...
summit_H3K27ac_LTR_breakdown <- anno_H3K27ac_summits %>% 
  left_join(H3K27ac_set_case_lookup) %>% 
   mutate(repClass = na_if(repClass, "")) %>% 
  dplyr::filter(repFamily %in% c("ERV1", "ERVK","ERVL", "ERVL-MaLR","Gypsy","LTR")) %>% 
  dplyr::select(Peakid, repClass,repFamily, repName, cluster:set_case)
summit_H3K27ac_LTR_breakdown 
```
```{r}
summit_H3K27ac_LTR_breakdown %>% 
