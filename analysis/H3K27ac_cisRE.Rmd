...
# 
# 
cCRE_discrete_colors <- paletteer::paletteer_d("ggsci::default_aaas")

cCRE_discrete_colors
```
...
# saveRDS(H3K27ac_cre_df,"data/RDS_files/H3K27ac_cre_df.RDS")
...
# test <- mergeByOverlaps(H3K27ac_summit_gr,SCREEN_hg38_cCREs)
# 
# test

SCREEN_hg38_cCREs
H3K27ac_cre_df%>% as.data.frame() %>%
  # dplyr::filter(cCRE_ID %in% c("EH38E1711087", "EH38E4104717", "EH38E3092695"))
  dplyr::filter(cCRE_ID %in% c("EH38E1535425", "EH38E2955081","EH38E2955082", "EH38E4035740", "EH38E2955083"))
# H3K27ac_cre_df %>% dplyr::filter(cCRE_ID =="EH38D4811366")
#   SCREEN_hg38_cCREs %>% as.data.frame()%>% dplyr::filter( DHS_ID=="EH38D4811366")
```
...
H3K27ac_cre_counts <- H3K27ac_cre_anno_full %>% 
 ...
 saveRDS(H3K27ac_cre_counts,"data/RDS_files/H3K27ac_cre_hg38_counts.RDS")
...
H3K27ac_heart_cre_df <- tibble(
  ...
  cCRE_ID   = mcols(SCREEN_heart_hg38_cCREs)$cCRE_ID[subjectHits(H3K27ac_heart_cre_hits)],
  cCRE_class= mcols(SCREEN_heart_hg38_cCREs)$cCRE_class[subjectHits(H3K27ac_heart_cre_hits)],
  cCRE_name = mcols(SCREEN_heart_hg38_cCREs)$name[subjectHits(H3K27ac_heart_cre_hits)])

screen_labels <-SCREEN_hg38_cCREs %>% 
  as.data.frame() %>% 
  dplyr::select(DHS_ID,cCRE_ID,cCRE_class)

H3K27ac_heart_cre_anno <-H3K27ac_anno_ROIs$all_H3K27ac %>% as.data.frame() %>% 
  left_join(.,H3K27ac_heart_cre_df ) %>% 
  dplyr::select(Peakid,cluster, cCRE_ID,cCRE_name) %>% 
    left_join(screen_labels, by = c("cCRE_name"="cCRE_ID")) %>% 
  mutate(cluster = if_else(is.na(cluster), "not_assigned", cluster), 
         cCRE_ID=if_else(is.na(cCRE_ID),"NA",cCRE_ID),
         cCRE_class=if_else(is.na(cCRE_class),"NA",cCRE_class))
...
H3K27ac_heart_cre_counts <- H3K27ac_heart_cre_anno %>% 
 ...
 ggtitle("Heart-left ventricle cCREs")+scale_fill_cCREs()+
   scale_fill_manual(values=ccre_cols_v4)
...
# H3K27ac_heart_cre_counts %>% ungroup %>% distinct (cCRE_ID)
```
```{r}
...
annotated_H3K27ac_summits %>% 
  dplyr::filter(cluster=="Set_2" &!is.na(summit_pos))
