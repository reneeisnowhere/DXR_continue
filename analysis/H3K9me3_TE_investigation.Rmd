...
I then overlapped ROIs with H3K9me3 ROIs to see how many unique H3K9me3 ROIs are involved.
```{r}
...
K27ac_only_K9me3_peakids <- K27ac_overlap_df %>% distinct(Peakid)

```
Now to filter and connect H3K9me3 ROIs, that overlap LTRs and at least one H3K9me3
```{r}
LTR_overlap_df %>% 
  dplyr::filter(Peakid %in% K27ac_only_K9me3_peakids$Peakid) %>%  distinct(RM_id)
```
