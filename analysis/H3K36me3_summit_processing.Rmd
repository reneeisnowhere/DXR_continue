---
title: "H3K36me3 summit processing"
...
H3K36me3_dir <- "C:/Users/renee/Other_projects_data/DXR_data/final_data/summit_files/H3K36me3" 

##pull all histone files together
H3K36me3_summit_files <- list.files(
  path = H3K36me3_dir,
  pattern = "\\.bed$",
  recursive = TRUE,
  full.names = TRUE
)
length(H3K36me3_summit_files)

# head(H3K36me3_summit_files)
```
...
all_H3K36me3_summits_list<- lapply(H3K36me3_summit_files, read_summit)
all_H3K36me3_summits_gr <- do.call(c, all_H3K36me3_summits_list)  # combine into one GRanges object
H3K36me3_lookup <- imap_dfr(peakAnnoList_H3K36me3[1:3], ~
  tibble(Peakid = .x@anno$Peakid, cluster = .y)
)


###Adding in sampleinfo dataframe
meta <- as.data.frame(mcols(all_H3K36me3_summits_gr))
meta2 <- meta %>% 
  left_join(., sampleinfo, by=c("Library_ID"="Library ID"))

mcols(all_H3K36me3_summits_gr) <- meta2

mcols(all_H3K36me3_summits_gr)$group <- 
  paste(all_H3K36me3_summits_gr$Treatment,
        all_H3K36me3_summits_gr$Timepoint,
        sep = "_")
ROIs <-  H3K36me3_sets_gr$all_H3K36me3
```
...
# 
#   # Expand each ROI symmetrically by max_dist
#   roi_expanded <- roi_unassigned
# start(roi_expanded) <- start(roi_expanded) - max_dist
# end(roi_expanded)   <- end(roi_expanded) + max_dist
# 
#   ov2 <- findOverlaps(roi_expanded, cons_chr)
# 
#   dn_df <- if (length(ov2) > 0) {
#     tibble(
#       Peakid = roi_unassigned$Peakid[queryHits(ov2)],
#       summit_pos = start(cons_chr)[subjectHits(ov2)],
#       summit_score = mcols(cons_chr)$score[subjectHits(ov2)]
#     ) %>%
#       group_by(Peakid) %>%
#       slice_max(summit_score, with_ties = FALSE) %>%
#       ungroup()
...
```{r H3K36me3 get highes per group funciton}
...
gr_by_group <- split(all_H3K36me3_summits_gr,
                                    all_H3K36me3_summits_gr$group)
# gr_by_group <- as(gr_by_group, "CompressedGRangesList")
### now reducing within some width by 100 bp with revmap
groups <- unique(all_H3K36me3_summits_gr$group)

reduced_groups <- lapply(groups, function(g) {
  gr_sub <- all_H3K36me3_summits_gr[all_H3K36me3_summits_gr$group == g]
...
```{r H3K36me3 reduced sets grouping}
reduced_sets <- list(
...
highest_summits_all <- lapply(reduced_sets, get_highest_per_group, orig_summits_gr = all_H3K36me3_summits_gr)
```
  
  Looking at number of summits across groups as a function of min.gap number

```{r H3K36me3 across groups mingap}
...
ggplot(summit_counts_group, aes(x = gap, y = n_summits, group = group, color = group)) +
  ...
  labs(
    title = "Effect of min-gap width on number of highest \nsummits per group; H3K36me3",
    x = "Min-gap width (bp)",
...
```{r H3K36me3 double reduce function}
...
```{r H3K36me3 calculating of double reduce}

# BPPARAM <- SnowParam(workers = 4, type = "SOCK")  
# register(BPPARAM)
# 
# heatmap_data <- double_reduce_summits(
#   summits_gr = all_H3K36me3_summits_gr,
#   ROIs = H3K36me3_sets_gr$all_H3K36me3,
#   groups = unique(all_H3K36me3_summits_gr$group),
...
# saveRDS(heatmap_data,"data/RDS_files/H3K36me3_heatmap_data.RDS")
heatmap_data <- readRDS("data/RDS_files/H3K36me3_heatmap_data.RDS")
...
```{r H3K36me3 plot mingap averages}
ggplot(heatmap_avg, aes(x = factor(min_gap_within),
  ...
  labs(
    ...
    title = "Effect of min-gap width on summits per ROI H3K36me3"
  ) +
...
# ### Step 1
# temp_highest_per_group <- get_highest_per_group_parallel(all_H3K36me3_summits_gr,
#                                                          group_col = "group",
#                                                          score_col = "score",
#                                                          min_gap_within = 100,
#                                                          workers = workers)
# 
# saveRDS(temp_highest_per_group,"data/RDS_files/H3K36me3_temp_highest_per_group.RDS")
temp_highest_per_group <- readRDS("data/RDS_files/H3K36me3_temp_highest_per_group.RDS")
...
# saveRDS(consensus_summits_gr,"data/RDS_files/H3K36me3_consensus_summits_gr.RDS")
consensus_summits_gr <- readRDS("data/RDS_files/H3K36me3_consensus_summits_gr.RDS")
final_data <- assign_best_summit_to_ROI_parallel(consensus_summits_gr, ROIs,max_dist = 100,workers = workers)

# saveRDS(final_data,"data/RDS_files/H3K36me3_final_data.RDS")
final_data <- readRDS("data/RDS_files/H3K36me3_final_data.RDS")
...
overlap_df <- tibble(
  ...
  summit_pos = start(all_H3K36me3_summits_gr)[subjectHits(ov)],
  summit_score = mcols(all_H3K36me3_summits_gr)$score[subjectHits(ov)]
)
...
complete_summit_df <- final_df %>% 
  ...
  distinct
# saveRDS(complete_summit_df,"data/RDS_files/H3K36me3_complete_summit_df.RDS")
...
# saveRDS(complete_summit_gr,"data/RDS_files/H3K36me3_complete_summit_gr.RDS")
```
...
# H3K36me3_set1_600 <- resize_and_trim(SET_1_gr,flank=300)
H3K36me3_set1_400 <- resize_and_trim(SET_1_gr,flank=200)
# H3K36me3_set1_600<- trim(H3K36me3_set1_600)
# H3K36me3_set1_600 <- H3K36me3_set1_600[width(H3K36me3_set1_600) > 0]

H3K36me3_set1_400 <- resize(SET_1_gr, width = 1 + 200*2, fix = "center")
H3K36me3_set1_400<- trim(H3K36me3_set1_400)
H3K36me3_set1_400 <- H3K36me3_set1_400[width(H3K36me3_set1_400) > 0]

rtracklayer::export(SET_1_gr, "data/Bed_exports/H3K36me3_Set_1_summits.bed")
# rtracklayer::export(H3K36me3_set1_600, "data/Bed_exports/H3K36me3_Set_1_600.bed")
rtracklayer::export(H3K36me3_set1_400, "data/Bed_exports/H3K36me3_Set_1_400.bed")

SET_2_gr <- complete_summit_gr[
  !is.na(mcols(complete_summit_gr)$cluster) &
  mcols(complete_summit_gr)$cluster == "Set_2"]

# H3K36me3_set2_600 <- resize_and_trim(SET_2_gr,flank=300)
H3K36me3_set2_400 <- resize_and_trim(SET_2_gr,flank=400)

# H3K36me3_set2_600 <- resize(SET_2_gr, width = 1 + 300*2, fix = "center")
# H3K36me3_set2_600 <- trim(H3K36me3_set2_600)
# H3K36me3_set2_600 <- H3K36me3_set2_600[width(H3K36me3_set2_600) > 0]

H3K36me3_set2_400 <- resize(SET_2_gr, width = 1 + 200*2, fix = "center")
H3K36me3_set2_400 <- trim(H3K36me3_set2_400)
H3K36me3_set2_400 <- H3K36me3_set2_400[width(H3K36me3_set2_400) > 0]
rtracklayer::export(SET_2_gr, "data/Bed_exports/H3K36me3_Set_2_summits.bed")
# rtracklayer::export(H3K36me3_set2_600, "data/Bed_exports/H3K36me3_Set_2_600.bed")
rtracklayer::export(H3K36me3_set2_400, "data/Bed_exports/H3K36me3_Set_2_400.bed")
# 
# SET_3_gr <- complete_summit_gr[
#   !is.na(mcols(complete_summit_gr)$cluster) &
#   mcols(complete_summit_gr)$cluster == "Set_3"]
# 
# H3K36me3_set3_600 <- resize_and_trim(SET_3_gr,flank=300)
# H3K36me3_set3_400 <- resize_and_trim(SET_3_gr,flank=200)
# 
# H3K36me3_set3_600 <- resize(SET_3_gr, width = 1 + 300*2, fix = "center")
# H3K36me3_set3_600 <- trim(H3K36me3_set3_600)
# H3K36me3_set3_600 <- H3K36me3_set3_600[width(H3K36me3_set3_600) > 0]
# 
# H3K36me3_set3_400 <- resize(SET_3_gr, width = 1 + 200*2, fix = "center")
# H3K36me3_set3_400 <- trim(H3K36me3_set3_400)
# H3K36me3_set3_400 <- H3K36me3_set3_400[width(H3K36me3_set3_400) > 0]
# 
# rtracklayer::export(SET_3_gr, "data/Bed_exports/H3K36me3_Set_3_summits.bed")
# rtracklayer::export(H3K36me3_set3_600, "data/Bed_exports/H3K36me3_Set_3_600.bed")
# rtracklayer::export(H3K36me3_set3_400, "data/Bed_exports/H3K36me3_Set_3_400.bed")
  

rtracklayer::export(complete_summit_gr, "data/Bed_exports/H3K36me3_complete_final_summits.bed")
...
for (nm in names(group_gr_list)) {
    outfile <- file.path(outdir, paste0(nm, "_H3K36me3_summits.bed"))
