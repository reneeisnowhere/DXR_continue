...
# saveRDS(SINE_df,"data/RDS_files/SINE_DF_hg38.RDS")
# saveRDS(LTR_df,"data/RDS_files/LTR_DF_hg38.RDS")
# saveRDS(DNA_df,"data/RDS_files/DNA_DF_hg38.RDS")
# saveRDS(LINE_df,"data/RDS_files/LINE_DF_hg38.RDS")
# saveRDS(SVA_df,"data/RDS_files/SVA_DF_hg38.RDS")
```
...
##assigning Peakid as name of summit region
 ...
 H3K27ac_ext200_gr <- H3K27ac_summit_gr_ext
...
dataf <- H3K27ac_ext200_gr
  ...
  All_H3K27ac <-peakAnnoList_H3K27ac$all_H3K27ac %>% 
  as.GRanges()
...
total_SINE_summits <- tibble(
  cluster = mcols(H3K27ac_summit_gr_ext)$cluster
...
not_SINE_tRNARTE_counts <- SINE_tRNARTE_counts %>%
  left_join(total_SINE_summits, by = "cluster") %>%
  mutate(count = total - count,
         status = "not_TE") %>%
  dplyr::select(cluster, repName, status, count)

SINE_tRNARTE_df_long <- bind_rows(SINE_tRNARTE_counts %>%
                            dplyr::select(cluster, repName, status, count),
                          not_SINE_tRNARTE_counts) %>% 
  filter(!is.na(cluster))

tRNARTE_SINE_results <- comparisons %>%
  mutate(results = map2(cluster2, cluster1, function(c2, c1) {

       SINE_tRNARTE_df_long %>%
        distinct(repName) %>%
        pull() %>%
        map_dfr(function(rep) {
          test_pair_TE_repName(
...
ggplot(tRNARTE_SINE_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  geom_point(aes(color = odds_ratio > 1), size = 3) +
  # geom_text_repel(data = subset(SVA_results, FDR < 0.05)) +
  geom_text_repel(data = subset(tRNARTE_SINE_results, FDR < 0.05),  # only significant points
    size = 3,
    max.overlaps = 100
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
  labs(
    x = "(Odds Ratio)",
    y = "-log10(FDR)",
    title = "SINE Name (tRNA-RTE) enrichment at ROI extended-summits\nH3K27ac"
  ) +
  theme_classic()+
  facet_wrap(~comparison)
```
...
not_SINE_tRNA_counts <- SINE_tRNA_counts %>%
  left_join(total_SINE_summits, by = "cluster") %>%
  mutate(count = total - count,
         status = "not_TE") %>%
  dplyr::select(cluster, repName, status, count)

SINE_tRNA_df_long <- bind_rows(SINE_tRNA_counts %>%
                            dplyr::select(cluster, repName, status, count),
                          not_SINE_tRNA_counts) %>% 
  filter(!is.na(cluster))

tRNA_SINE_results <- comparisons %>%
  mutate(results = map2(cluster2, cluster1, function(c2, c1) {
...
ggplot(tRNA_SINE_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  geom_point(aes(color = odds_ratio > 1), size = 3) +
  # geom_text_repel(data = subset(SVA_results, FDR < 0.05)) +
  geom_text_repel(data = subset(tRNA_SINE_results, FDR < 0.05),  # only significant points
    size = 3,
    max.overlaps = 100
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
  labs(
    x = "(Odds Ratio)",
    y = "-log10(FDR)",
    title = "SINE Name (tRNA) enrichment at ROI extended-summits\nH3K27ac"
  ) +
  theme_classic()+
  facet_wrap(~comparison)
```
...
LINE_overlap_df <- tibble(
  ...
  cluster   = mcols(H3K27ac_summit_gr_ext)$cluster[queryHits(LINE_hits)],
...
```{r LINE subfam RTE_BovB}

LINE_hits <- findOverlaps(H3K27ac_summit_gr_ext, LINE_gr, ignore.strand = TRUE)
LINE_name_overlap_df <- tibble(
  LINE_id = queryHits(LINE_hits),
  cluster   = mcols(H3K27ac_summit_gr_ext)$cluster[queryHits(LINE_hits)],
  TE_type   = mcols(LINE_gr)$repFamily[subjectHits(LINE_hits)],
  repName= mcols(LINE_gr)$repName[subjectHits(LINE_hits)])

LINE_fams <- LINE_name_overlap_df %>% 
  group_by(TE_type) %>% 
  distinct(TE_type)
##L2, L1*, CR1,RTE-BovB*, RTE-X,Dong-R4,Penelope

LINE_RTE_BovB_counts <-
  LINE_name_overlap_df %>%
    dplyr::filter(TE_type=="RTE-BovB") %>% 
  count(cluster, repName, name = "count") %>%
  mutate(status = "TE")
...
not_LINE_RTE_BovB_counts <- LINE_RTE_BovB_counts %>%
  left_join(total_LINE_summits, by = "cluster") %>%
  mutate(count = total - count,
         status = "not_TE") %>%
  dplyr::select(cluster, repName, status, count)

LINE_RTE_BovB_df_long <- bind_rows(LINE_RTE_BovB_counts %>%
                            dplyr::select(cluster, repName, status, count),
                          not_LINE_RTE_BovB_counts) %>% 
  filter(!is.na(cluster))

RTE_BovB_LINE_results <- comparisons %>%
  mutate(results = map2(cluster2, cluster1, function(c2, c1) {

       LINE_RTE_BovB_df_long %>%
        distinct(repName) %>%
        pull() %>%
        map_dfr(function(rep) {
          test_pair_TE_repName(
...
ggplot(RTE_BovB_LINE_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  geom_point(aes(color = odds_ratio > 1), size = 3) +
  # geom_text_repel(data = subset(SVA_results, FDR < 0.05)) +
  geom_text_repel(data = subset(RTE_BovB_LINE_results, FDR < 0.05),  # only significant points
    size = 3,
    max.overlaps = 100
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
  labs(
    x = "(Odds Ratio)",
    y = "-log10(FDR)",
    title = "LINE Name (RTE-BovB) enrichment at ROI extended-summits\nH3K27ac"
  ) +
  theme_classic()+
  facet_wrap(~comparison)
```
...
ERV1_ERVLMaLR_results <- comparisons %>%
  mutate(results = map2(cluster2, cluster1, function(c2, c1) {
...
ggplot(ERV1_ERVLMaLR_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  geom_point(aes(color = odds_ratio > 1), size = 3) +
  # geom_text_repel(data = subset(SVA_results, FDR < 0.05)) +
  geom_text_repel(data = subset(ERV1_ERVLMaLR_results, FDR < 0.05),  # only significant points
    size = 3,
    max.overlaps = 100
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
...
DNA_TcMarTigger_counts <-
  DNA_name_overlap_df %>%
    dplyr::filter(TE_type=="TcMar-Tigger") %>% 
  count(cluster, repName, name = "count") %>%
  mutate(status = "TE")
...
not_DNA_TcMarTigger_counts <- DNA_TcMarTigger_counts %>%
  left_join(total_DNA_summits, by = "cluster") %>%
  mutate(count = total - count,
         status = "not_TE") %>%
  dplyr::select(cluster, repName, status, count)

DNA_TcMarTigger_df_long <- bind_rows(DNA_TcMarTigger_counts %>%
                            dplyr::select(cluster, repName, status, count),
                          not_DNA_TcMarTigger_counts) %>% 
  filter(!is.na(cluster))

TcMarTigger_DNA_results <- comparisons %>%
  mutate(results = map2(cluster2, cluster1, function(c2, c1) {

       DNA_TcMarTigger_df_long %>%
        distinct(repName) %>%
        pull() %>%
        map_dfr(function(rep) {
          test_pair_TE_repName(
...
ggplot(TcMarTigger_DNA_results, aes(x = (odds_ratio), y = -log10(FDR),label = repName)) +
  geom_point(aes(color = odds_ratio > 1), size = 3) +
  # geom_text_repel(data = subset(SVA_results, FDR < 0.05)) +
  geom_text_repel(data = subset(TcMarTigger_DNA_results, FDR < 0.05),  # only significant points
    size = 3,
    max.overlaps = 100
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
  labs(
    x = "(Odds Ratio)",
    y = "-log10(FDR)",
    title = "DNA Name (TcMar-Tigger) enrichment at ROI extended-summits\nH3K27ac"
  ) +
  theme_classic()+
  facet_wrap(~comparison)
```
...
anno_H3K27ac_ext200 <- readRDS("data/TE_annotation/annotated_H3K27ac_ext200_TE_overlaps.RDS")
anno_H3K27ac_ext200 %>% 
...
H3K27ac_te_summary <- anno_H3K27ac_ext200 %>% 
...
ggplot(fam_pie_df, aes(x = "", y = percent, fill = repClass)) +
  ...
  labs(
    title = "TE Class composition among summit-ext TE-overlapping H3K27ac ROIs",
    fill = "TE Class")
...
ggplot(H3K27ac_te_family_clust, aes(x = cluster, y=percent, fill=repClass))+
  ...
  theme_bw() +
  labs(
    title = "TE Class composition among H3K27ac summit-exit TEs per set",
    fill = "TE family"
  )
  ggplot(H3K27ac_te_family_clust, aes(x = cluster, y=percent, fill=repClass))+
   geom_col(width = 0.8, color = "white") +
...
H3K27ac_long_repFamily_ext <-H3K27ac_long_repFamily %>% 
  rbind((LTR_genome %>% mutate(cluster="hg38")))

H3K27ac_long_repFamily_ext
```
...
repName_counts <- annotated_H3K27ac_ext200 %>%
...
# saveRDS(repName_results,"data/RDS_files/H3K27ac_repName_results.RDS")
```
...
for (group_name in set_list) {
 ...
 ##exporting the repeatmaskerclean file for enrichment testing

rtracklayer::export(rmskr_std_gr,"data/Bed_exports/repeatmasker_clean.bed", format="BED")
```
