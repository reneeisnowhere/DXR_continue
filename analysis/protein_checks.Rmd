...
# saveRDS(module_ids,"data/Other_paper_data/Protein_paper_assigned_lookup.RDS")
```
...
fisher.test(tab, alternative = "greater") 
```
Investigation of number of proteins in alpha
...
tibble(background_genes) %>% 
  ...
  group_by(is_alpha, is_set3) %>% tally

set2vset1alpha_mat <- matrix(c(31,1234,516,21345),nrow=2,byrow = TRUE)
fisher.test(set2vset1alpha_mat)

set3vset1alpha_mat <- matrix(c(143,4710,516,21345),nrow=2,byrow = TRUE)
fisher.test(set3vset1alpha_mat)

set3vset2alpha_mat <- matrix(c(143,4710,31,1234),nrow=2,byrow = TRUE)
fisher.test(set3vset2alpha_mat)

```
### MER61E specific tests
...
Well, disappointing that no MER61E assigned near genes overlap a DAP.  (Assignment of neargenes was done by GREAT website app)
...
# saveRDS(save_MER,"data/MER61intermediatemarking_protein.RDS")
```
