...
H3K9me3_summit_df <- readRDS("data/RDS_files/H3K9me3_complete_summit_df.RDS")
```
...
### All H3K9me3 ROIs overlapping a TE at summits

overlapping summits and all TEs dataframe making: 
```{r}
dataf <- H3K9me3_summit_gr
# H3K9me3_summit_ols <-
...
H3K9me3_summit_Peakids <- H3K9me3_summit_ols %>% 
    distinct(Peakid)
  # saveRDS(H3K9me3_summit_ols,"data/RDS_files/H3K9me3_summit_ROI_TE_overlaps.RDS")
```
...
annotated_H3K9me3_summits <- All_H3K9me3 %>%
  left_join(., all_summits_df) %>% 
    left_join(., (H3K9me3_summit_ols %>% 
                  as.data.frame() %>% 
                  group_by(Peakid) %>% 
                  summarize(., 
          ...
          mutate(SVA_status = case_when(is.na(repClass)~ "wnot_SVA",
                                        ...
                                        TRUE ~"wnot_SVA")) %>% 
  left_join(H3K9me3_lookup, by = c("Peakid","cluster")) %>%
  mutate(cluster = if_else(is.na(cluster), "not_assigned", cluster)) 

# saveRDS(annotated_H3K9me3_summits,"data/TE_annotation/H3K9me3_annotated_summit_TE_overlaps.RDS")
```
...
```{r SINE Overlapping}
sine_hits <- findOverlaps(H3K9me3_summit_gr, SINE_gr, ignore.strand = TRUE)
...
```{r LINE Overlapping}
LINE_hits <- findOverlaps(H3K9me3_summit_gr, LINE_gr, ignore.strand = TRUE)
...
#### Overlapping LINE family with ROIs to get a count
...
```{r LTR Overlapping}
LTR_hits <- findOverlaps(H3K9me3_summit_gr, LTR_gr, ignore.strand = TRUE)
...
#### Overlapping LTR family with ROIs to get a count
...
```{r SVA Overlapping}
SVA_hits <- findOverlaps(H3K9me3_summit_gr, SVA_gr, ignore.strand = TRUE)
...
ggplot(SVA_results, aes(x = (odds_ratio), y = -log10(FDR),label = TE_type)) +
  ...
  facet_wrap(~comparison)
```
#### Overlapping SVA family with ROIs to get a count
...
```{r DNA Overlapping}
DNA_hits <- findOverlaps(H3K9me3_summit_gr, DNA_gr, ignore.strand = TRUE)
...
out_dir <- "data/Bed_exports/H3K9me3_sets"
...
#### Overlapping DNA family with ROIs to get a count
...
```{r}
# H3K9me3_set_case_lfc <- readRDS("data/RDS_files/H3K9me3_set_case_lfc.RDS")

annotated_tables <- H3K9me3_sets_gr$all_H3K9me3_regions %>% as.data.frame()

anno_H3K9me3_summits <- readRDS("data/TE_annotation/H3K9me3_annotated_summit_TE_overlaps.RDS")

anno_H3K9me3 <- annotated_tables


# H3K9me3_set_case_lookup <- bind_rows(H3K9me3_set_case_lfc, .id = "set_case") %>% 
...
anno_H3K9me3 %>% 
  ...
  ylab("proportion")+
  ggtitle("Proportion of ROIs by cluster, H3K9me3")+
  theme_bw() +
  theme(
...
H3K9me3_te_summary_summits <- anno_H3K9me3_summits %>% 
  left_join(H3K9me3_lookup) %>% 
...
pie_df_summits <- H3K9me3_te_summary_summits %>% 
  count(category) %>% 
  mutate(
    percent = n / sum(n) * 100
  )
...
```{r class summit}
H3K9me3_te_family_long_summits <- H3K9me3_te_summary_summits %>% 
  filter(!is.na(repClass), repClass != "") %>%   # TE-positive only
  separate_rows(repClass, sep = ";") %>% 
  distinct(Peakid, repClass,cluster)


fam_pie_df_summits <- H3K9me3_te_family_long_summits %>% 
...
H3K9me3_long_repFamily_summits <- H3K9me3_te_summary_summits %>% 
  filter(!is.na(repFamily), repFamily != "") %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
  distinct(Peakid, repFamily)


H3K9me3_LTR_pie_df_summits <- H3K9me3_long_repFamily_summits %>% 
  dplyr::filter(repFamily %in% c("ERV1", "ERVK","ERVL", "ERVL-MaLR","Gypsy","LTR")) %>%
  count(repFamily, name = "n") %>% 
  mutate(percent= n/ sum(n) * 100)

ggplot(H3K9me3_LTR_pie_df_summits, aes(x = "", y = percent, fill = repFamily)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  theme_void() +
  geom_text_repel(
...
H3K9me3_te_family_clust_summits <- H3K9me3_te_family_long_summits %>% 
...
ggplot(H3K9me3_te_family_clust_summits, aes(x = "", y=percent, fill=repClass))+
  geom_col(width =1, color="white")+
  coord_polar(theta = "y")+
  geom_text_repel(
...
ggplot(H3K9me3_te_family_clust_summits, aes(x = cluster, y=percent, fill=repClass))+
...
H3K9me3_long_repFamily_summits <-
H3K9me3_te_summary_summits %>% 
   dplyr::filter(cluster != "not_assigned") %>%
  filter(!is.na(repFamily), repFamily != "") %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
...
ggplot(H3K9me3_long_repFamily_summits, aes(x = "", y=percent, fill=repFamily))+
  geom_col(width =1, color="white")+
  coord_polar(theta = "y")+
    geom_text_repel(
    aes(label = sprintf("%s\n%.1f%%", repFamily, percent)),
  ...
  theme_void()+
  labs(
    title = "TE family relative abundance among ROI summits by set H3K9me3",
    fill = "TE family"
  )


ggplot(H3K9me3_long_repFamily_summits, aes(x = cluster, y=percent, fill=repFamily))+
   geom_col(width = 0.8, color = "white") +
  geom_text_repel(
    aes(label = sprintf("%s\n%.1f%%", repFamily, percent)),
    position = position_stack(vjust = 0.5),
  ...
  theme_bw() +
  labs(
    title = "TE Family composition among TE-overlapping ROI summits per set H3K9me3",
    fill = "TE family"
  )
...
LTR_bar_df_summits <-H3K9me3_te_summary_summits %>%
  filter(!is.na(repFamily), repFamily != "")  %>%   # TE-positive only
  separate_rows(repFamily, sep = ";") %>% 
  distinct(cluster,Peakid, repFamily) %>% 
  dplyr::filter(repFamily %in% c("ERV1", "ERVK","ERVL", "ERVL-MaLR","Gypsy","LTR")) %>%
...
#### Looking at ERV1s in set2and3

```{r}

anno_H3K9me3_summits %>% 
  dplyr::select(summit_id, summit_pos, Peakid,repClass:repName, ID) %>% 
  mutate(repClass = na_if(repClass, "")) %>% 
  na.omit(repClass)
